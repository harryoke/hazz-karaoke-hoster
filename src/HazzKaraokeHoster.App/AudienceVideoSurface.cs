...
// Audience-only decoder. Audio and the private preview remain on
...
public sealed class AudienceVideoSurface : Grid
{
    ...
    public Uri? Source
    {
        ...
        set
        {
            ...
            if ((!UseLibVlc || WindowsRequested || _fallback) && _vlc is not null)
                ReleaseNative();
            _fallback = false;
            EnsureBackend();
    ...
    {
        if (_closed || _fallback || _player is not null || WindowsRequested || !UseLibVlc) return;
        try
        {
