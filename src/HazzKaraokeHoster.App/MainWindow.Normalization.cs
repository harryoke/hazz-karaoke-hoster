...
{
    private sealed class NormalizationSettings { public bool Enabled { get; set; } public double TargetDb { get; set; } = -18; public double MaxBoostDb { get; set; } = 12; }
    ...
    private void LoadNormalization()
    {
        try
        {
            ...
            AudioNormalization.MaxBoostDb = settings?.MaxBoostDb ?? 12;
        }
    ...
    {
        var window = new Window { Owner = this, Title = "Automatic Loudness Matching", Width = 470, SizeToContent = SizeToContent.Height, WindowStartupLocation = WindowStartupLocation.CenterOwner };
        var panel = new StackPanel { Margin = new Thickness(20) }; window.Content = panel;
        var enabled = new CheckBox { Content = "AUTOMATIC LOUDNESS MATCHING — KARAOKE + MUSIC", IsChecked = AudioNormalization.Enabled, Margin = new Thickness(0,0,0,16) };
        ...
        var boostLabel = new TextBlock { Margin = new Thickness(0,12,0,0) }; panel.Children.Add(boostLabel);
        var boost = new Slider { Minimum = 0, Maximum = 12, TickFrequency = 1, IsSnapToTickEnabled = true, Value = AudioNormalization.MaxBoostDb }; panel.Children.Add(boost);
        void Apply()
        {
            ...
            AudioNormalization.MaxBoostDb = boost.Value;
            boostLabel.Text = $"Maximum boost for quiet tracks: +{boost.Value:0} dB";
            label.Text = $"Target level: {AudioNormalization.TargetDb:0} dB RMS";
        }
        enabled.Checked += (_, _) => Apply(); enabled.Unchecked += (_, _) => Apply(); level.ValueChanged += (_, _) => Apply(); boost.ValueChanged += (_, _) => Apply(); Apply();
        var close = new Button { Content = "SAVE AND CLOSE", MinHeight = 32, Margin = new Thickness(0,16,0,0) }; panel.Children.Add(close);
        close.Click += (_, _) => window.Close();
        window.Closed += (_, _) => { try { File.WriteAllText(NormalizationPath + ".tmp", JsonSerializer.Serialize(new NormalizationSettings { Enabled = AudioNormalization.Enabled, TargetDb = AudioNormalization.TargetDb, MaxBoostDb = AudioNormalization.MaxBoostDb })); File.Move(NormalizationPath + ".tmp", NormalizationPath, true); } catch (Exception ex) { App.WriteDiagnostic("NORMALIZATION SAVE", ex.ToString()); } };
        window.ShowDialog();
    }
}
